...
locals {
  ...
  v2_template_annotations = {
    for k, v in local.template_annotations : k => v
    if !can(regex("^run.googleapis.com/(vpc-access-connector|vpc-access-egress)$", k))
  }
}
...
resource "google_cloud_run_v2_service" "main" {
  ...
  template {
    ...
    dynamic "vpc_access" {
      for_each = (var.vpc_connector != null || var.vpc_egress != null) ? [1] : []
    ...
    annotations = local.v2_template_annotations
      ...
      content {
        ...
        dynamic "secret" {
          ...
          content {
            ...
            items {
              version = secret.value.items["key"]
    ...
    containers {
      ...
      dynamic "env" {
        ...
        content {
          ...
          value_source {
            secret_key_ref {
              secret  = env.value.value_from.secret_key_ref.name
              version = env.value.value_from.secret_key_ref.key
            }
          }
        }
      }
  ...
  dynamic "traffic" {
    ...
    content {
      ...
      revision = lookup(traffic.value, "latest_revision", false) ? self.latest_ready_revision : lookup(traffic.value, "revision_name", null)
    }
  }

  lifecycle {
    ignore_changes = [
      template[0].labels["goog-managed-by"],
    ]
  }
}
